...
def maxFrequency(listofmarks):
    frequency_dict = {}  # Create an empty dictionary to store mark frequencies
    max_frequency = 0
    most_frequent_marks = []

    # Calculate mark frequencies
    ...
    # Find marks with the maximum frequency
    for mark, frequency in frequency_dict.items():
        if frequency == max_frequency:
            most_frequent_marks.append(mark)

    # Print the marks with the highest frequency
    print("Marks  |  Frequency")
    for mark in most_frequent_marks:
        print(mark, "    |  ", max_frequency)
